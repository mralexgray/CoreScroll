...
- (CGFloat)contentWidth {	return [ self.layoutManager preferredSizeOfLayer:self].width; 	}

- (CGFloat)visibleWidth {	return [self visibleRect].size.width;	}

- (void)scrollToPosition:(CGFloat)position {

	CGFloat positionPercentage = position;
	if ( position < 0 )	 positionPercentage = 0;
	if ( position > 1 )  positionPercentage = 1;
